...
# main window
root = Tk()         #root is our nain window
root.title("Rock Paper Scissors Game")  #title of main window
root.configure(background="#9b59b6")  #Background colour ,purple colour value
...
#till now we imported all our pictures , now we have to insert it.

# insert picture
user_label = Label(root, image=scissor_img, bg="#9b59b6")         #root is our nain window
...
# update user score
def updateUserScore():
    score = int(playerScore["text"])  #converting the score numerical string text to int
    score += 1                        #updating the score by one
    playerScore["text"] = str(score)  #converting the int score again into string , as it was earlier type 
...
def updateChoice(x):
    ...
    # for computer
    compChoice = choices[randint(0, 2)]  #this will generate random number from 0 to 2
    if compChoice == "rock":
